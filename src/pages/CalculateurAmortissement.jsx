...
const COEF_DEGRESSIF = { 3:1.25, 4:1.25, 5:1.75, 6:1.75, 7:2.25 };
...
function calculerLineaire(valeur, duree, dateAcquisition) {
  ...
  const moisRestants = 12 - dateAcq.getMonth(); // prorata 1ère année

  for (let i = 0; i < duree; i++) {
    const annee = dateAcq.getFullYear() + i;
    const dotation = i === 0
      ? (valeur * taux * moisRestants / 12)
      : valeur * taux;
...
function calculerDegressif(valeur, duree, dateAcquisition) {
  ...
  for (let i = 0; i < duree; i++) {
    ...
    const tauxLineaireRest = 1 / anneesRestantes;
    const taux = Math.max(tauxDegressif, tauxLineaireRest);
    const dotation = i === 0
      ? vna * tauxDegressif * moisRestants / 12
      : vna * taux;
    const dotationReelle = Math.min(dotation, vna);
    vna = Math.max(0, vna - dotationReelle);
    lignes.push({ annee, dotation: dotationReelle, vna, taux: taux * 100 });
...
export default function CalculateurAmortissement() {
  ...
  const duree = categorie === CATEGORIES.length - 1 && dureeCustom
    ? parseInt(dureeCustom) : cat.duree;

  const tableau = useMemo(() => {
    const v = parseFloat(valeur);
    if (!v || v <= 0 || !duree || duree <= 0 || !dateAcq) return [];
    return methode === 'lineaire'
      ? calculerLineaire(v, duree, dateAcq)
      : calculerDegressif(v, duree, dateAcq);
  }, [valeur, duree, methode, dateAcq]);

  const totalDotations = tableau.reduce((s, l) => s + l.dotation, 0);

  // SVG graphique barres
    ...
    <div style={{ fontFamily:"'Nunito Sans', sans-serif", padding:'32px 28px', maxWidth:900, margin:'0 auto' }}>
      ...
      <div style={{ marginBottom:28 }}>
        ...
        <p style={{ fontSize:13, color:C.light, marginTop:4 }}>
          Calcul linéaire ou dégressif · Tableau annuel · Conforme PCG français
        </p>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div style={{ background:'#fff', border:`1px solid ${C.border}`, borderRadius:14, padding:22, boxShadow:'0 1px 6px rgba(0,0,0,0.05)' }}>
            <label style={lS}>Valeur d'acquisition HT (€)</label>
            <input type="number" value={valeur} onChange={e => setValeur(e.target.value)} placeholder="Ex : 2500" style={iS} min="0"/>
            <p style={{ fontSize:11, color:C.light, marginTop:6, marginBottom:0 }}>
              ⚠️ Seuls les biens {'>'} 500 € HT sont amortissables (règle fiscale française)
            </p>
          </div>
          ...
          <div style={{ background:'#fff', border:`1px solid ${C.border}`, borderRadius:14, padding:22, boxShadow:'0 1px 6px rgba(0,0,0,0.05)' }}>
            ...
            <select value={categorie} onChange={e => setCategorie(Number(e.target.value))} style={iS}>
              {CATEGORIES.map((c, i) => (
                <option key={i} value={i}>{c.label} {i < CATEGORIES.length-1 ? `(${c.duree} ans)` : ''}</option>
              ))}
            </select>
          ...
          <div style={{ background:'#fff', border:`1px solid ${C.border}`, borderRadius:14, padding:22, boxShadow:'0 1px 6px rgba(0,0,0,0.05)' }}>
            <label style={lS}>Méthode d'amortissement</label>
            {METHODES.map(m => (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          {tableau.length > 0 ? (<>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
              {[
                { label:'Valeur amortie',  value: euro(totalDotations), color: C.blue  },
                { label:'Durée',           value: `${duree} ans`,       color: C.dark  },
                { label:'Taux annuel',     value: pct(100/duree),       color: C.orange},
                { label:'Coef. dégressif', value: methode==='degressif' ? `×${getCoef(duree)}` : '—', color: C.green },
              ].map(k => (
                <div key={k.label} style={{ background:'#fff', border:`1px solid ${C.border}`, borderRadius:12, padding:'14px 16px', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
                  <div style={{ fontSize:11, color:C.light, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:6 }}>{k.label}</div>
            ...
            <div style={{ background:'#fff', border:`1px solid ${C.border}`, borderRadius:14, padding:20, boxShadow:'0 1px 6px rgba(0,0,0,0.05)' }}>
              ...
              <svg width="100%" viewBox={`0 0 480 ${svgH + 30}`} style={{ overflow:'visible' }}>
                {tableau.map((l, i) => {
                  const barH = (l.dotation / maxDot) * svgH;
                  const x = i * (480 / tableau.length) + (480 / tableau.length - barW) / 2;
                  const y = svgH - barH;
                  return (
                    <g key={i}>
                      <rect x={x} y={y} width={barW} height={barH} rx={4}
                        fill={methode==='lineaire' ? C.blue : C.orange} opacity={0.8}/>
                ...
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  <thead>
                    <tr style={{ background:C.bg }}>
                      {['Année','Taux','Dotation','VNA'].map(h => (
                        <th key={h} style={{ padding:'10px 16px', fontSize:11, fontWeight:700, color:C.light, textAlign:'right', textTransform:'uppercase', letterSpacing:'0.05em', borderBottom:`1px solid ${C.border}` }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div style={{ background:'rgba(212,168,83,0.06)', border:'1px solid rgba(212,168,83,0.2)', borderRadius:12, padding:'12px 16px', fontSize:12, color:'#92713A', lineHeight:1.6 }}>
              ⚠️ Ce calcul est fourni à titre indicatif. Les durées d'amortissement peuvent varier selon votre situation. Consultez votre expert-comptable pour valider votre plan d'amortissement.
            </div>
