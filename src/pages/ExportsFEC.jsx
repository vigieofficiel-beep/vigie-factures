...
const Icon = {
  ...
  Check: () => <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12" strokeLinecap="round"/></svg>,
  ...
  Book: () => <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg>,
};

// ─── Constantes FEC ──────────────────────────────────────────────────
// Comptes PCG simplifiés pour le mapping automatique
const COMPTES = {
...
function genererEcrituresFEC(recettes, depenses, factures_fourn, transactions, siren) {
  ...
  const ligne = (journalCode, journalLib, ecritureNum, date, compte, compteLib, compteAux, compteAuxLib, debit, credit, libelle, dateValidation, montantDevise, idevise) => {
    return [
      cleanStr(journalCode),
      cleanStr(journalLib),
      ...
      cleanStr(compteLib),
      cleanStr(compteAux),
      cleanStr(compteAuxLib),
      formatMontantFEC(debit),
      formatMontantFEC(credit),
      cleanStr(libelle),
      formatDateFEC(dateValidation || date),
      montantDevise ? formatMontantFEC(montantDevise) : "",
      idevise || "",
    ].join("|");
  };

  // ── RECETTES (Journal VE - Ventes)
  recettes.forEach((r) => {
    const ht = Number(r.montant_ht || r.montant || 0);
    const tva = ht * (Number(r.taux_tva || 20) / 100);
    const ttc = ht + tva;
    ...
    if (tva > 0) {
      ecritures.push(ligne("VE", "Journal des ventes", numJournal, r.date, COMPTES.tva_collectee.num, COMPTES.tva_collectee.lib, "", "", 0, tva, `TVA - ${lib}`, r.date, "", ""));
    }
    numJournal++;
  });

  // ── DEPENSES (Journal AC - Achats)
  depenses.forEach((d) => {
    const ht = Number(d.montant_ht || d.montant || 0);
    const tva = ht * (Number(d.taux_tva || 20) / 100);
    const ttc = ht + tva;
    const lib = `Dépense - ${d.description || d.libelle || d.categorie || "Achat"}`;

    // Choix du compte de charge selon catégorie
    let compteCharge = COMPTES.depense_default;
    const cat = (d.categorie || "").toLowerCase();
    if (cat.includes("transport") || cat.includes("kilomètre")) compteCharge = COMPTES.depense_transport;
    else if (cat.includes("repas") || cat.includes("restaurant")) compteCharge = COMPTES.depense_repas;
    else if (cat.includes("logiciel") || cat.includes("abonnement") || cat.includes("saas")) compteCharge = COMPTES.depense_logiciel;
    else if (cat.includes("matériel") || cat.includes("equipement")) compteCharge = COMPTES.depense_materiel;

    // Débit charge HT
    ecritures.push(ligne("AC", "Journal des achats", numJournal, d.date, compteCharge.num, compteCharge.lib, "", "", ht, 0, lib, d.date, "", ""));
    // Débit TVA déductible
    if (tva > 0) {
      ecritures.push(ligne("AC", "Journal des achats", numJournal, d.date, COMPTES.tva_deductible.num, COMPTES.tva_deductible.lib, "", "", tva, 0, `TVA - ${lib}`, d.date, "", ""));
    }
    // Crédit fournisseur
  ...
  // ── FACTURES FOURNISSEURS (Journal AC)
  factures_fourn.forEach((f) => {
    const ht = Number(f.montant_ht || 0);
    const tva = ht * (Number(f.tva || 20) / 100);
    const ttc = ht + tva;
    ...
    if (tva > 0) {
      ecritures.push(ligne("AC", "Journal des achats", numJournal, f.date_facture, COMPTES.tva_deductible.num, COMPTES.tva_deductible.lib, "", "", tva, 0, `TVA - ${lib}`, f.date_facture, "", ""));
    }
  ...
  // ── TRANSACTIONS BANCAIRES (Journal BQ)
  transactions.forEach((t) => {
    const montant = Math.abs(Number(t.montant || 0));
    const lib = `${t.libelle || t.description || "Transaction"}`;
    const isCredit = Number(t.montant) > 0;

    ecritures.push(ligne("BQ", "Journal de banque", numJournal, t.date, COMPTES.banque.num, COMPTES.banque.lib, "", "",
      isCredit ? montant : 0,
      isCredit ? 0 : montant,
      lib, t.date, "", ""));
    ecritures.push(ligne("BQ", "Journal de banque", numJournal, t.date,
      isCredit ? COMPTES.client.num : COMPTES.fournisseur.num,
      isCredit ? COMPTES.client.lib : COMPTES.fournisseur.lib,
      "", "",
      isCredit ? 0 : montant,
      isCredit ? montant : 0,
      lib, t.date, "", ""));
...
export default function ExportsFEC() {
  ...
  const [periodeType, setPeriodeType] = useState("annee"); // "annee" | "mois"
  ...
  // Sources de données à inclure
  const [sources, setSources] = useState({
    recettes: true,
    depenses: true,
    factures_fournisseurs: true,
    transactions: true,
  });
  ...
  const getDateRange = () => {
    const debut = periodeType === "annee"
      ? `${annee}-01-01`
      : `${annee}-${String(moisDebut + 1).padStart(2, "0")}-01`;
    const fin = periodeType === "annee"
      ? `${annee}-12-31`
      : new Date(annee, moisFin + 1, 0).toISOString().split("T")[0];
    return { debut, fin };
  };
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const totalRecettes = r.reduce((s, x) => s + Number(x.montant_ht || x.montant || 0), 0);
      const totalDepenses = d.reduce((s, x) => s + Number(x.montant_ht || x.montant || 0), 0);
      const totalFactures = f.reduce((s, x) => s + Number(x.montant_ht || 0), 0);

      setStats({
        nbRecettes: r.length, totalRecettes,
        nbDepenses: d.length, totalDepenses,
        nbFactures: f.length, totalFactures,
        nbTransactions: t.length,
        total: r.length + d.length + f.length + t.length,
      });

      // Générer aperçu
      const ecritures = genererEcrituresFEC(r, d, f, t, siren);
      setPreview(ecritures.slice(0, 10));
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const exporterFEC = async () => {
    ...
    try {
      ...
      const ecritures = genererEcrituresFEC(
        recettes || [], depenses || [], factures || [], transactions || [], siren
      );
      ...
      const contenu = [entete, ...ecritures].join("\n");
      const blob = new Blob(["\uFEFF" + contenu], { type: "text/plain;charset=utf-8;" });

      const nomFichier = siren
        ? `${siren}FEC${annee}1231.txt`
        : `FEC_${annee}_export.txt`;
  ...
  return (
    <div style={{ padding: 32, maxWidth: 900, margin: "0 auto", fontFamily: "'Outfit', 'Segoe UI', sans-serif" }}>

      {/* En-tête */}
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: "#111827" }}>Exports FEC</h1>
      ...
      <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 12, padding: "14px 18px", marginBottom: 24, display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ fontSize: 13, color: "#1e40af", lineHeight: 1.6 }}>
          Le FEC est un fichier obligatoire remis à l'administration fiscale lors d'un contrôle. Il contient l'ensemble des écritures comptables de l'exercice dans un format normalisé. Ce module consolide vos recettes, dépenses et transactions bancaires au format requis.
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Identité société */}
          <Section titre="Identité de la société">
            <Field label="SIREN (9 chiffres)">
              <input value={siren} onChange={e => setSiren(e.target.value.replace(/\D/g, "").slice(0, 9))}
                style={inputStyle} placeholder="123456789" maxLength={9} />
            </Field>
            ...
            <Field label="Granularité">
              <div style={{ display: "flex", gap: 8 }}>
                {[{ id: "annee", label: "Année complète" }, { id: "mois", label: "Période personnalisée" }].map(o => (
                  <button key={o.id} onClick={() => setPeriodeType(o.id)} style={{
                    flex: 1, padding: "7px 10px", borderRadius: 8, border: "1px solid",
                    borderColor: periodeType === o.id ? "#6366f1" : "#d1d5db",
                    background: periodeType === o.id ? "#eef2ff" : "#fff",
                    color: periodeType === o.id ? "#4f46e5" : "#374151",
                    fontWeight: periodeType === o.id ? 700 : 400,
            ...
            {periodeType === "mois" && (
              <>
                <Field label="Mois de début">
                  <select value={moisDebut} onChange={e => setMoisDebut(Number(e.target.value))} style={inputStyle}>
                    {MOIS_FR.map((m, i) => <option key={i} value={i}>{m}</option>)}
                  </select>
                </Field>
                <Field label="Mois de fin">
                  <select value={moisFin} onChange={e => setMoisFin(Number(e.target.value))} style={inputStyle}>
                    {MOIS_FR.map((m, i) => <option key={i} value={i}>{m}</option>)}
                  </select>
                </Field>
              </>
          ...
          <Section titre="Sources de données">
            {[
              { k: "recettes", label: "Recettes / Factures clients" },
              { k: "depenses", label: "Dépenses & Frais" },
              { k: "factures_fournisseurs", label: "Factures fournisseurs" },
              { k: "transactions", label: "Transactions bancaires" },
            ].map(s => (
              <div key={s.k} onClick={() => toggleSource(s.k)} style={{
                display: "flex", alignItems: "center", gap: 10,
                padding: "10px 12px", borderRadius: 8, cursor: "pointer",
                border: `1px solid ${sources[s.k] ? "#6366f1" : "#e5e7eb"}`,
                background: sources[s.k] ? "#eef2ff" : "#f9fafb",
                transition: "all 150ms ease",
              }}>
                <div style={{
                  width: 20, height: 20, borderRadius: 6, flexShrink: 0,
                  background: sources[s.k] ? "#6366f1" : "#e5e7eb",
                  display: "flex", alignItems: "center", justifyContent: "center",
                }}>
                  {sources[s.k] && <span style={{ color: "#fff", fontSize: 12 }}>✓</span>}
                </div>
                <span style={{ fontSize: 14, color: sources[s.k] ? "#4f46e5" : "#6b7280", fontWeight: sources[s.k] ? 600 : 400 }}>
                  {s.label}
                </span>
              </div>
            ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Récapitulatif */}
          <Section titre="Récapitulatif de la période">
            {loading ? (
                  ...
                  <div key={s.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "#f9fafb", borderRadius: 8, border: "1px solid #e5e7eb" }}>
                    ...
                    {s.montant !== null && (
                      <div style={{ fontSize: 14, fontWeight: 700, color: s.color }}>
                        {s.montant.toLocaleString("fr-FR", { style: "currency", currency: "EUR" })}
                      </div>
                    )}
                  </div>
                ))}
          ...
          {/* Bouton export */}
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <button onClick={exporterFEC} disabled={loading || !stats || stats.total === 0} style={{
              display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
              background: stats && stats.total > 0 ? "#6366f1" : "#e5e7eb",
              color: stats && stats.total > 0 ? "#fff" : "#9ca3af",
              border: "none", padding: "14px 20px", borderRadius: 10,
              fontWeight: 700, fontSize: 15, cursor: stats && stats.total > 0 ? "pointer" : "not-allowed",
            ...
            {siren && (
              <div style={{ fontSize: 12, color: "#6b7280", textAlign: "center" }}>
                Nom du fichier : <span style={{ fontFamily: "monospace", color: "#374151" }}>{siren}FEC{annee}1231.txt</span>
              </div>
            )}
            ...
            <div style={{ background: "#fefce8", border: "1px solid #fde68a", borderRadius: 10, padding: "12px 14px", display: "flex", gap: 10 }}>
              ...
              <p style={{ margin: 0, fontSize: 12, color: "#92400e", lineHeight: 1.6 }}>
                Ce fichier est généré à titre préparatoire. Il doit être vérifié et validé par votre expert-comptable avant toute remise à l'administration fiscale. Vigie Pro est un outil de pré-comptabilité.
              </p>
            </div>
          </div>
...
function Section({ titre, children }) {
  return (
    <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
      <h3 style={{ margin: "0 0 14px", fontSize: 14, fontWeight: 700, color: "#111827", textTransform: "uppercase", letterSpacing: "0.05em" }}>{titre}</h3>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {children}
      </div>
    </div>
