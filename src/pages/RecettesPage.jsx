...
const STATUTS = [
  { id:'brouillon', label:'Brouillon', color:'rgba(237,232,219,0.4)', bg:'rgba(255,255,255,0.06)' },
  ...
  { id:'annule',    label:'Annulé',    color:'rgba(237,232,219,0.2)', bg:'rgba(255,255,255,0.03)' },
];
...
function genererPDF(devis, client, profil, lignes) {
  ...
  doc.text('TVA :',xTot,y); doc.setTextColor(...dark); doc.text(`${totalTVA.toFixed(2)} €`,W-M-2,y,{align:'right'}); y+=2;
...
function LignesPrestation({ lignes, setLignes }) {
  ...
  const iS = { padding:'8px 10px', borderRadius:7, border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.06)', color:'#EDE8DB', fontSize:12, outline:'none', width:'100%', boxSizing:'border-box' };
  ...
  return (
    <div style={{marginBottom:14}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'3fr 0.6fr 1fr 0.8fr 1fr 28px',gap:6,marginBottom:6,padding:'0 4px'}}>
        {['Description','Qté','P.U. HT','TVA %','Total TTC',''].map((h,i) => (
          <span key={i} style={{fontSize:10,fontWeight:700,color:'rgba(237,232,219,0.3)',textTransform:'uppercase',letterSpacing:'0.05em'}}>{h}</span>
        ))}
      </div>
      {lignes.map((l,i) => {
        ...
        return (
          <div key={i} style={{display:'grid',gridTemplateColumns:'3fr 0.6fr 1fr 0.8fr 1fr 28px',gap:6,marginBottom:6,alignItems:'center'}}>
            <input value={l.description} onChange={e=>updateLigne(i,'description',e.target.value)} style={iS} placeholder="Description de la prestation"/>
            <input type="number" min="1" value={l.quantite} onChange={e=>updateLigne(i,'quantite',e.target.value)} style={iS}/>
            <input type="number" step="0.01" min="0" value={l.prix_unitaire} onChange={e=>updateLigne(i,'prix_unitaire',e.target.value)} style={iS} placeholder="0.00"/>
            <select value={l.tva_taux} onChange={e=>updateLigne(i,'tva_taux',e.target.value)} style={{...iS,cursor:'pointer'}}>
...
function ClientForm({ onSave, onCancel, workspaceId=null }) {
  ...
  const handleSubmit=async(e)=>{
    ...
    try{
      ...
      const{error:err}=await supabasePro.from('clients').insert({...form,user_id:user.id,workspace_id:workspaceId});
  ...
  return (
    <form onSubmit={handleSubmit} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,padding:24,marginBottom:24}}>
      ...
      <div style={{display:'flex',gap:10}}>
        ...
        <button type="button" onClick={onCancel} style={{padding:'10px 18px',borderRadius:9,border:'1px solid rgba(255,255,255,0.1)',background:'rgba(255,255,255,0.04)',color:'rgba(237,232,219,0.5)',fontSize:13,cursor:'pointer'}}>Annuler</button>
      </div>
    </form>
  );
}
...
function DevisForm({ clients, onSave, onCancel, editData=null, prefill=null, workspaceId=null }) {
  ...
  const [form,setForm]=useState(editData||{
    ...
    date_validite:in30, date_echeance:in30, statut:'brouillon',
    notes: prefill?.description ?? '',
  });
  const [lignes,setLignes]=useState(()=>{
    if(prefill?.montant_ttc) return [{description:prefill.description||'',quantite:1,prix_unitaire:prefill.montant_ht||prefill.montant_ttc||'',tva_taux:prefill.tva?Math.round((prefill.tva/(prefill.montant_ht||1))*100):20}];
  ...
  const handleSubmit=async(e)=>{
    ...
    try{
      const{data:{user}}=await supabasePro.auth.getUser();
      const payload={...form,user_id:user.id,workspace_id:workspaceId,montant_ht:totalHT,tva_taux:lignes[0]?.tva_taux||20,montant_ttc:totalHT+totalTVA,description:JSON.stringify(lignes)};
  ...
  return (
    <form onSubmit={handleSubmit} style={{background:'rgba(255,255,255,0.04)',border:`1px solid ${prefill?`${ACCENT}40`:'rgba(255,255,255,0.08)'}`,borderRadius:14,padding:24,marginBottom:24}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
        ...
        <div><label style={lS}>Date d'émission *</label><input type="date" value={form.date_emission} onChange={set('date_emission')} required style={{...iS,colorScheme:'dark'}}/></div>
        <div><label style={lS}>Date de validité <Tooltip text={TIPS.date_validite}/></label><input type="date" value={form.date_validite} onChange={set('date_validite')} style={{...iS,colorScheme:'dark'}}/></div>
        <div><label style={lS}>Échéance paiement <Tooltip text={TIPS.date_echeance}/></label><input type="date" value={form.date_echeance} onChange={set('date_echeance')} style={{...iS,colorScheme:'dark'}}/></div>
      ...
      <div style={{background:'rgba(212,168,83,0.06)',border:'1px solid rgba(212,168,83,0.2)',borderRadius:8,padding:'8px 12px',marginBottom:16,fontSize:11,color:'#D4A853'}}>⚠️ Les montants sont indicatifs. Validez avec votre expert-comptable avant envoi.</div>
      {error&&<div style={{color:'#C75B4E',fontSize:12,marginBottom:12}}>{error}</div>}
      <div style={{display:'flex',gap:10}}>
...
export default function RecettesPage() {
  ...
  useEffect(()=>{
    ...
    try {
      ...
      if(raw){const data=JSON.parse(raw);if(data.source==='prohome_ocr'&&data.type_document==='recette'){setOcrPrefill(data);setShowDevisForm(true);sessionStorage.removeItem('ocr_prefill');}}
    }catch{}
  },[activeWorkspace]);
  ...
  const envoyerRelance=async(d)=>{const count=(d.relance_count||0)+1;await supabasePro.from('devis').update({relance_count:count,derniere_relance:new Date().toISOString().split('T')[0]}).eq('id',d.id);alert(`Relance ${count} enregistrée pour ${d.clients?.nom}`);fetchAll();};
  ...
  const devisExport=filtered.map(d=>({numero:d.numero,client:d.clients?.nom||'',date_emission:d.date_emission,date_echeance:d.date_echeance,montant_ht:d.montant_ht,tva_taux:d.tva_taux,montant_ttc:d.montant_ttc,statut:d.statut,relances:d.relance_count||0}));

  return (
    <div style={{fontFamily:"'Nunito Sans', sans-serif",padding:'32px 28px',maxWidth:1000,margin:'0 auto'}}>
      {profilIncomplet&&(
        <div onClick={()=>navigate('/pro/profil')} style={{display:'flex',alignItems:'center',gap:10,background:'rgba(212,168,83,0.06)',border:'1px solid rgba(212,168,83,0.2)',borderRadius:10,padding:'10px 16px',marginBottom:20,cursor:'pointer'}}>
          <AlertTriangle size={14} color="#D4A853"/>
          <span style={{fontSize:12,color:'#D4A853',fontWeight:600}}>Profil pro incomplet — vos PDF ne seront pas correctement remplis.</span>
          <span style={{fontSize:12,color:'#D4A853',fontWeight:700,marginLeft:'auto'}}>Compléter →</span>
        </div>
      )}
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24,flexWrap:'wrap',gap:12}}>
        ...
        <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
          <button onClick={()=>navigate('/pro/profil')} style={{display:'flex',alignItems:'center',gap:6,padding:'9px 16px',borderRadius:9,border:'1px solid rgba(255,255,255,0.1)',background:'rgba(255,255,255,0.04)',color:'rgba(237,232,219,0.5)',fontSize:12,fontWeight:600,cursor:'pointer'}}><Settings size={13}/> Mon profil</button>
          ...
          <button onClick={()=>{setShowClientForm(true);setShowDevisForm(false);}} style={{display:'flex',alignItems:'center',gap:6,padding:'9px 16px',borderRadius:9,border:`1px solid ${ACCENT}`,background:'rgba(91,163,199,0.08)',color:ACCENT,fontSize:12,fontWeight:700,cursor:'pointer'}}><Users size={13}/> Nouveau client</button>
      ...
      {showClientForm&&<ClientForm workspaceId={activeWorkspace?.id} onSave={()=>{setShowClientForm(false);fetchAll();}} onCancel={()=>setShowClientForm(false)}/>}
      ...
      <div style={{display:'flex',gap:4,marginBottom:18,background:'rgba(255,255,255,0.05)',borderRadius:10,padding:4,width:'fit-content'}}>
        {[{id:'devis',label:'Devis',icon:FileText},{id:'clients',label:'Clients',icon:Users}].map(t=>{const Icon=t.icon;return(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{display:'flex',alignItems:'center',gap:6,padding:'7px 16px',borderRadius:8,border:'none',background:tab===t.id?'rgba(91,163,199,0.15)':'transparent',color:tab===t.id?ACCENT:'rgba(237,232,219,0.4)',fontSize:13,fontWeight:tab===t.id?700:500,cursor:'pointer',transition:'all 150ms ease'}}>
      ...
      {tab==='devis'&&(<>
        <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
          {[{id:'tous',label:'Tous',color:'rgba(237,232,219,0.4)'},...STATUTS].map(s=>(
            <div key={s.id} style={{display:'flex',alignItems:'center',gap:4}}>
              <button onClick={()=>setFilterStatut(s.id)} style={{padding:'5px 14px',borderRadius:20,border:`1px solid ${filterStatut===s.id?s.color:'rgba(255,255,255,0.1)'}`,background:filterStatut===s.id?`${s.color}20`:'transparent',color:filterStatut===s.id?s.color:'rgba(237,232,219,0.4)',fontSize:12,fontWeight:filterStatut===s.id?700:500,cursor:'pointer'}}>{s.label}</button>
        ...
        <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,overflow:'hidden'}}>
          ...
          :filtered.length===0?<div style={{padding:48,textAlign:'center'}}><FileText size={32} color="rgba(255,255,255,0.1)" style={{marginBottom:12}}/><p style={{color:'rgba(237,232,219,0.3)',fontSize:13,margin:0}}>{devis.length===0?'Aucun devis — cliquez sur "Nouveau devis"':'Aucun devis pour ce filtre'}</p></div>
          :<table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead>
              <tr style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
                {['N° Devis','Client','Émission','Échéance','Montant TTC','Statut','Relances',''].map((h,i)=>(
            ...
            <tbody>
              {filtered.map((d,i)=>{
                ...
                return(<tr key={d.id||i} style={{borderBottom:'1px solid rgba(255,255,255,0.04)',background:isLate?'rgba(199,91,78,0.03)':'transparent'}}
                  onMouseEnter={ev=>ev.currentTarget.style.background=isLate?'rgba(199,91,78,0.05)':'rgba(255,255,255,0.03)'}
                  onMouseLeave={ev=>ev.currentTarget.style.background=isLate?'rgba(199,91,78,0.03)':'transparent'}>
                  ...
                  <td style={{padding:'11px 14px',fontSize:12,color:'rgba(237,232,219,0.4)'}}>{formatDate(d.date_emission)}</td>
                  ...
                  <td style={{padding:'11px 14px'}}>
                    <div style={{display:'flex',gap:4}}>
                      <button onClick={()=>telechargerPDF(d)} title="PDF" style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'rgba(237,232,219,0.2)'}} onMouseEnter={ev=>ev.currentTarget.style.color=ACCENT} onMouseLeave={ev=>ev.currentTarget.style.color='rgba(237,232,219,0.2)'}><FileDown size={13}/></button>
                      ...
                      <button onClick={()=>{setEditDevis(d);setShowDevisForm(false);}} title="Modifier" style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'rgba(237,232,219,0.2)'}} onMouseEnter={ev=>ev.currentTarget.style.color=ACCENT} onMouseLeave={ev=>ev.currentTarget.style.color='rgba(237,232,219,0.2)'}><Edit2 size={13}/></button>
                      <button onClick={()=>deleteDevis(d.id)} title="Supprimer" style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'rgba(237,232,219,0.2)'}} onMouseEnter={ev=>ev.currentTarget.style.color='#C75B4E'} onMouseLeave={ev=>ev.currentTarget.style.color='rgba(237,232,219,0.2)'}><Trash2 size={13}/></button>
                    </div>
                  </td>
                </tr>);
              })}
      ...
      {tab==='clients'&&(
        <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,overflow:'hidden'}}>
          {clients.length===0?<div style={{padding:48,textAlign:'center'}}><Users size={32} color="rgba(255,255,255,0.1)" style={{marginBottom:12}}/><p style={{color:'rgba(237,232,219,0.3)',fontSize:13,margin:0}}>Aucun client — cliquez sur "Nouveau client"</p></div>
          :<table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead>
              <tr style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
                {['Nom','Email','Téléphone','SIRET',''].map((h,i)=>(
                  <th key={i} style={{padding:'11px 14px',textAlign:'left',fontSize:10,fontWeight:700,color:'rgba(237,232,219,0.3)',textTransform:'uppercase',letterSpacing:'0.06em'}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {clients.map((c,i)=>(
                <tr key={c.id||i} style={{borderBottom:'1px solid rgba(255,255,255,0.04)'}}
                  ...
                  <td style={{padding:'11px 14px',fontSize:12,color:'rgba(237,232,219,0.5)'}}>{c.email}</td>
                  <td style={{padding:'11px 14px',fontSize:12,color:'rgba(237,232,219,0.5)'}}>{c.telephone||'—'}</td>
                  <td style={{padding:'11px 14px',fontSize:12,color:'rgba(237,232,219,0.5)'}}>{c.siret||'—'}</td>
                  <td style={{padding:'11px 14px'}}><button onClick={()=>deleteClient(c.id)} style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'rgba(237,232,219,0.2)'}} onMouseEnter={ev=>ev.currentTarget.style.color='#C75B4E'} onMouseLeave={ev=>ev.currentTarget.style.color='rgba(237,232,219,0.2)'}><Trash2 size={13}/></button></td>
                </tr>
              ))}
            </tbody>
          </table>}
