...
const STATUTS = [
  ...
  { id: 'annule',     label: 'Annulé',      color: 'rgba(237,232,219,0.2)' },
];

const STATUT_TIPS = {
  brouillon: TIPS.statut_brouillon, envoye: TIPS.statut_envoye, signe: TIPS.statut_signe,
  encaisse: TIPS.statut_encaisse, en_retard: TIPS.statut_en_retard, annule: TIPS.statut_annule,
};
  ...
  return (
    <div style={{marginBottom:14}}>
      ...
      <div style={{marginTop:10,padding:'10px 14px',background:'rgba(255,255,255,0.04)',borderRadius:9,border:'1px solid rgba(255,255,255,0.08)',display:'flex',justifyContent:'flex-end',gap:24}}>
        <span style={{fontSize:12,color:'rgba(237,232,219,0.4)',display:'flex',alignItems:'center',gap:4}}>HT : <strong style={{color:'#EDE8DB'}}>{totalHT.toFixed(2)} €</strong></span>
        <span style={{fontSize:12,color:'rgba(237,232,219,0.4)',display:'flex',alignItems:'center',gap:4}}>TVA : <strong style={{color:'#EDE8DB'}}>{totalTVA.toFixed(2)} €</strong></span>
...
function ClientForm({ onSave, onCancel }) {
  ...
  const handleSubmit=async(e)=>{
    ...
    try {
      ...
      const user=session?.user; if(!user)throw new Error('Session expirée');
      const{error:err}=await supabasePro.from('clients').insert({ nom:form.nom, courriel:form.email, 'téléphone':form.telephone, adresse:form.adresse, siret:form.siret, user_id:user.id });
  ...
  return (
    <form onSubmit={handleSubmit} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,padding:24,marginBottom:24}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
        <div><label style={lS}>Nom *</label><input value={form.nom} onChange={set('nom')} required style={iS} placeholder="Ex: SARL Dupont"/></div>
        <div><label style={lS}>Courriel *</label><input type="email" value={form.email} onChange={set('email')} required style={iS} placeholder="contact@client.fr"/></div>
      ...
      <div style={{display:'flex',gap:10}}>
        <button type="submit" disabled={loading} style={{flex:1,padding:'10px',borderRadius:9,border:'none',background:ACCENT,color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer'}}>{loading?'Enregistrement...':'✓ Enregistrer'}</button>
...
function DevisForm({ clients, onSave, onCancel, editData=null, prefill=null, workspaceId=null }) {
  ...
  const [form,setForm]=useState(editData||{ client_id:'', numero:`DEV-${new Date().getFullYear()}${String(new Date().getMonth()+1).padStart(2,'0')}-001`, date_emission:prefill?.date?parseOCRDate(prefill.date):today, date_validite:in30, date_echeance:in30, statut:'brouillon', notes:prefill?.description??'' });
  const [lignes,setLignes]=useState(()=>{ if(prefill?.montant_ttc)return[{description:prefill.description||'',quantite:1,prix_unitaire:prefill.montant_ht||prefill.montant_ttc||'',tva_taux:20}]; if(editData?.description){try{return JSON.parse(editData.description);}catch{}} return[{description:'',quantite:1,prix_unitaire:'',tva_taux:20}]; });
  ...
  const handleSubmit=async(e)=>{
    e.preventDefault();setLoading(true);
    try {
      const{data:{session}}=await supabasePro.auth.getSession(); const user=session?.user; if(!user)throw new Error('Session expirée');
      const payload={...form,user_id:user.id,workspace_id:workspaceId||null,montant_ht:totalHT,tva_taux:lignes[0]?.tva_taux||20,montant_ttc:totalHT+totalTVA,description:JSON.stringify(lignes)};
      if(editData?.id){const{error:err}=await supabasePro.from('devis').update(payload).eq('id',editData.id);if(err)throw err;}
      else{const{error:err}=await supabasePro.from('devis').insert(payload);if(err)throw err;}
      onSave();
    }catch(err){setError(err.message);} setLoading(false);
  };
  const iS={width:'100%',padding:'9px 12px',borderRadius:8,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',color:'#EDE8DB',fontSize:13,outline:'none',boxSizing:'border-box'};
  ...
  return (
    <form onSubmit={handleSubmit} style={{background:'rgba(255,255,255,0.04)',border:`1px solid ${prefill?`${ACCENT}40`:'rgba(255,255,255,0.08)'}`,borderRadius:14,padding:24,marginBottom:24}}>
      {prefill&&<div style={{display:'flex',alignItems:'center',gap:8,padding:'10px 14px',borderRadius:9,background:`${ACCENT}10`,border:`1px solid ${ACCENT}30`,marginBottom:18}}><CheckCircle size={14} color={ACCENT}/><span style={{fontSize:13,fontWeight:600,color:ACCENT}}>Formulaire pré-rempli depuis l'analyse du document</span></div>}
      <h3 style={{fontSize:15,fontWeight:700,color:'#EDE8DB',marginBottom:20}}>{editData?'Modifier le document':'Nouveau document commercial'}</h3>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
        ...
        <div><label style={lS}>N° document *</label><input value={form.numero} onChange={set('numero')} required style={iS}/></div>
      ...
      <div style={{marginBottom:14}}><label style={lS}>Notes internes</label><input value={form.notes} onChange={set('notes')} style={iS} placeholder="Commentaires internes"/></div>
...
export default function RecettesPage() {
  ...
  useEffect(()=>{ const statutURL=searchParams.get('statut'); if(statutURL)setFilterStatut(statutURL); }, []);
  useEffect(()=>{
    fetchAll();
    try { const raw=sessionStorage.getItem('ocr_prefill'); if(raw){const data=JSON.parse(raw); if(data.source==='prohome_ocr'&&data.type_document==='recette'){setOcrPrefill(data);setShowDevisForm(true);sessionStorage.removeItem('ocr_prefill');}}} catch {}
  },[activeWorkspace]);

  const fetchAll=async()=>{
    setLoading(true);
    const{data:{session}}=await supabasePro.auth.getSession(); const user=session?.user; if(!user)return;
    ...
    const[{data:d},{data:c},{data:p}]=await Promise.all([qDevis,supabasePro.from('clients').select('*').eq('user_id',user.id).order('nom'),supabasePro.from('user_profiles').select('*').eq('id',user.id).single()]);
  ...
  const statutColor=(s)=>STATUTS.find(x=>x.id===s)?.color||'rgba(237,232,219,0.4)';

  return (
    <div style={{fontFamily:"'Nunito Sans', sans-serif",padding:isMobile?'16px 12px':'32px 28px',maxWidth:1000,margin:'0 auto'}}>
      {profilIncomplet&&(<div onClick={()=>navigate('/pro/profil')} style={{display:'flex',alignItems:'center',gap:10,background:'#FFF7ED',border:'1px solid #FED7AA',borderRadius:10,padding:'10px 16px',marginBottom:20,cursor:'pointer'}}><AlertTriangle size={14} color="#F59E0B"/><span style={{fontSize:12,color:'#92400E',fontWeight:600}}>Profil incomplet — vos PDF ne seront pas correctement remplis.</span><span style={{fontSize:12,color:'#F59E0B',fontWeight:700,marginLeft:'auto'}}>Compléter →</span></div>)}

      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:10}}>
        <h1 style={{fontFamily:"'Cormorant Garamond', serif",fontSize:isMobile?22:26,fontWeight:600,color:'#EDE8DB',margin:0}}>Recettes</h1>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          {!isMobile&&<button onClick={()=>navigate('/pro/profil')} style={{display:'flex',alignItems:'center',gap:6,padding:'9px 16px',borderRadius:9,border:'1px solid rgba(255,255,255,0.08)',background:'rgba(255,255,255,0.04)',color:'rgba(237,232,219,0.5)',fontSize:12,fontWeight:600,cursor:'pointer'}}><Settings size={13}/> Mon profil</button>}
          {!isMobile&&<DateFilter onChange={setDateRange} color={ACCENT}/>}
          {!isMobile&&<ExportButton data={filtered.map(d=>({numero:d.numero,client:d.clients?.nom||'',date_emission:d.date_emission,montant_ttc:d.montant_ttc,statut:d.statut}))} filename={`recettes-${new Date().getFullYear()}`} color={ACCENT} columns={[{key:'numero',label:'N°'},{key:'client',label:'Client'},{key:'date_emission',label:'Date'},{key:'montant_ttc',label:'TTC (€)'},{key:'statut',label:'Statut'}]}/>}
          <button onClick={()=>{setShowClientForm(true);setShowDevisForm(false);}} style={{display:'flex',alignItems:'center',gap:6,padding:'9px 14px',borderRadius:9,border:`1px solid ${ACCENT}`,background:'rgba(255,255,255,0.04)',color:ACCENT,fontSize:12,fontWeight:700,cursor:'pointer'}}><Users size={13}/>{isMobile?' Client':' Nouveau client'}</button>
          <button onClick={()=>{setShowDevisForm(true);setShowClientForm(false);setEditDevis(null);setOcrPrefill(null);}} style={{display:'flex',alignItems:'center',gap:6,padding:'9px 14px',borderRadius:9,border:'none',background:ACCENT,color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer'}}><Plus size={13}/>{isMobile?' Document':' Nouveau document'}</button>
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(130px, 1fr))',gap:10,marginBottom:20}}>
        {[{label:'Signés',value:formatEuro(totalSigne),color:'#5BC78A',icon:CheckCircle},{label:'Encaissé',value:formatEuro(totalEncaisse),color:'#A85BC7',icon:TrendingUp},{label:'En retard',value:enRetard.length,color:'#C75B4E',icon:AlertTriangle,alert:enRetard.length>0},{label:'Clients',value:clients.length,color:ACCENT,icon:Users}].map(s=>{const Icon=s.icon;return(
          <div key={s.label} style={{background:'rgba(255,255,255,0.04)',border:`1px solid ${s.alert?'rgba(199,91,78,0.3)':'rgba(255,255,255,0.08)'}`,borderRadius:12,padding:'12px 14px'}}>
            ...
            <p style={{fontSize:isMobile?16:20,fontWeight:700,color:s.alert?'#C75B4E':s.color,margin:0}}>{s.value}</p>
          </div>
        );})}
      </div>

      {showClientForm&&<ClientForm onSave={()=>{setShowClientForm(false);fetchAll();}} onCancel={()=>setShowClientForm(false)}/>}
      {(showDevisForm||editDevis)&&<DevisForm clients={clients} editData={editDevis} prefill={ocrPrefill} workspaceId={activeWorkspace?.id} onSave={()=>{setShowDevisForm(false);setEditDevis(null);setOcrPrefill(null);fetchAll();}} onCancel={()=>{setShowDevisForm(false);setEditDevis(null);setOcrPrefill(null);}}/>}
      ...
      <div style={{display:'flex',gap:4,marginBottom:16,background:'rgba(255,255,255,0.06)',borderRadius:10,padding:4,width:'fit-content'}}>
        {[{id:'devis',label:'Documents',icon:FileText},{id:'clients',label:'Clients',icon:Users}].map(t=>{const Icon=t.icon;return(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{display:'flex',alignItems:'center',gap:6,padding:'7px 14px',borderRadius:8,border:'none',background:tab===t.id?'rgba(91,163,199,0.15)':'transparent',color:tab===t.id?'#5BA3C7':'rgba(237,232,219,0.4)',fontSize:13,fontWeight:tab===t.id?700:500,cursor:'pointer',transition:'all 150ms ease'}}>
      ...
      {tab==='devis'&&(<>
        <div style={{display:'flex',gap:6,marginBottom:14,flexWrap:'wrap'}}>
          {[{id:'tous',label:'Tous',color:'rgba(237,232,219,0.4)'},...STATUTS].map(s=>(
            <button key={s.id} onClick={()=>setFilterStatut(s.id)} style={{padding:'5px 12px',borderRadius:20,border:`1px solid ${filterStatut===s.id?s.color:'rgba(255,255,255,0.1)'}`,background:filterStatut===s.id?`${s.color}15`:'transparent',color:filterStatut===s.id?s.color:'rgba(237,232,219,0.5)',fontSize:11,fontWeight:filterStatut===s.id?700:500,cursor:'pointer',whiteSpace:'nowrap'}}>
              {s.label}
            </button>
          ))}
        </div>
        ...
        {!isMobile&&(
          <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,overflow:'hidden'}}>
            ...
            :filtered.length===0?<div style={{padding:48,textAlign:'center'}}><p style={{color:'rgba(237,232,219,0.4)',fontSize:13,margin:0}}>{devis.length===0?'Aucun document':'Aucun document pour ce filtre'}</p></div>
            :<table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
                {['N° Document','Client','Émission','Échéance','Montant TTC','Statut','Relances',''].map((h,i)=>(<th key={i} style={{padding:'11px 14px',textAlign:'left',fontSize:10,fontWeight:700,color:'rgba(237,232,219,0.4)',textTransform:'uppercase',letterSpacing:'0.06em'}}>{h}</th>))}
              </tr></thead>
              <tbody>{filtered.map((d,i)=>{
                ...
                return(<tr key={d.id||i} style={{borderBottom:'1px solid rgba(255,255,255,0.04)',background:isLate?'rgba(199,91,78,0.02)':'transparent'}} onMouseEnter={ev=>ev.currentTarget.style.background=isLate?'rgba(199,91,78,0.04)':'rgba(255,255,255,0.04)'} onMouseLeave={ev=>ev.currentTarget.style.background=isLate?'rgba(199,91,78,0.02)':'transparent'}>
                  ...
                  <td style={{padding:'11px 14px'}}><select value={d.statut} onChange={e=>changerStatut(d.id,e.target.value)} style={{background:'transparent',border:'none',cursor:'pointer',fontSize:12,fontWeight:600,outline:'none',color:statutColor(d.statut)}}>{STATUTS.map(s=><option key={s.id} value={s.id}>{s.label}</option>)}</select></td>
                  ...
                  <td style={{padding:'11px 14px'}}>
                    <div style={{display:'flex',gap:4}}>
                      <button onClick={()=>telechargerPDF(d)} style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'rgba(237,232,219,0.4)'}} onMouseEnter={ev=>ev.currentTarget.style.color=ACCENT} onMouseLeave={ev=>ev.currentTarget.style.color='rgba(237,232,219,0.4)'}><FileDown size={13}/></button>
                      {(d.statut==='signe'||d.statut==='envoye')&&<button onClick={()=>envoyerRelance(d)} style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'#5BC78A'}}><RefreshCw size={13}/></button>}
                      <button onClick={()=>{setEditDevis(d);setShowDevisForm(false);}} style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'rgba(237,232,219,0.4)'}} onMouseEnter={ev=>ev.currentTarget.style.color=ACCENT} onMouseLeave={ev=>ev.currentTarget.style.color='rgba(237,232,219,0.4)'}><Edit2 size={13}/></button>
                      <button onClick={()=>deleteDevis(d.id)} style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'rgba(237,232,219,0.2)'}} onMouseEnter={ev=>ev.currentTarget.style.color='#C75B4E'} onMouseLeave={ev=>ev.currentTarget.style.color='#D0D4DC'}><Trash2 size={13}/></button>
                    </div>
                  </td>
                </tr>);})}
              </tbody>
        ...
        {isMobile&&(
          <div>
            ...
            :filtered.length===0?<div style={{padding:40,textAlign:'center'}}><p style={{color:'rgba(237,232,219,0.3)',fontSize:13,margin:0}}>{devis.length===0?'Aucun document — appuyez sur "Document"':'Aucun document pour ce filtre'}</p></div>
            :filtered.map((d,i)=>{
              const days=daysUntil(d.date_echeance); const isLate=(d.statut==='signe'||d.statut==='envoye')&&days!==null&&days<0;
              return(
                <div key={d.id||i} style={{background:isLate?'rgba(199,91,78,0.05)':'rgba(255,255,255,0.04)',border:`1px solid ${isLate?'rgba(199,91,78,0.3)':'rgba(255,255,255,0.08)'}`,borderRadius:14,padding:'14px 16px',marginBottom:10}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
                    <div><div style={{fontSize:13,fontWeight:700,color:'#EDE8DB',marginBottom:2}}>{d.clients?.nom||'—'}</div><div style={{fontSize:11,color:'rgba(237,232,219,0.4)'}}>{d.numero} · {formatDate(d.date_emission)}</div></div>
                    <div style={{fontSize:16,fontWeight:800,color:ACCENT}}>{formatEuro(d.montant_ttc)}</div>
                  </div>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <select value={d.statut} onChange={e=>changerStatut(d.id,e.target.value)} style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:8,padding:'4px 8px',fontSize:11,fontWeight:600,color:statutColor(d.statut),cursor:'pointer',outline:'none'}}>
                      {STATUTS.map(s=><option key={s.id} value={s.id}>{s.label}</option>)}
                    </select>
                    <div style={{display:'flex',gap:6}}>
                      <button onClick={()=>telechargerPDF(d)} style={{background:'rgba(91,163,199,0.1)',border:'none',borderRadius:6,padding:'6px 8px',cursor:'pointer',color:ACCENT,display:'flex'}}><FileDown size={13}/></button>
                      {(d.statut==='signe'||d.statut==='envoye')&&<button onClick={()=>envoyerRelance(d)} style={{background:'rgba(91,199,138,0.1)',border:'none',borderRadius:6,padding:'6px 8px',cursor:'pointer',color:'#5BC78A',display:'flex'}}><RefreshCw size={13}/></button>}
                      <button onClick={()=>{setEditDevis(d);setShowDevisForm(false);}} style={{background:'rgba(255,255,255,0.06)',border:'none',borderRadius:6,padding:'6px 8px',cursor:'pointer',color:'rgba(237,232,219,0.5)',display:'flex'}}><Edit2 size={13}/></button>
                      <button onClick={()=>deleteDevis(d.id)} style={{background:'rgba(199,91,78,0.08)',border:'none',borderRadius:6,padding:'6px 8px',cursor:'pointer',color:'#C75B4E',display:'flex'}}><Trash2 size={13}/></button>
                    </div>
                  </div>
                  {isLate&&<div style={{marginTop:8,fontSize:11,color:'#C75B4E',fontWeight:600}}>⚠️ En retard de {Math.abs(days)} jour{Math.abs(days)>1?'s':''}</div>}
                </div>
              );})}
          </div>
        )}
      ...
      {tab==='clients'&&(
        <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,overflow:'hidden'}}>
          {clients.length===0
            ?<div style={{padding:48,textAlign:'center'}}><Users size={32} color="#E8EAF0" style={{marginBottom:12}}/><p style={{color:'rgba(237,232,219,0.4)',fontSize:13,margin:0}}>Aucun client</p></div>
            :isMobile
              ?<div style={{padding:'8px'}}>{clients.map((c,i)=>(
                  <div key={c.id||i} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:'12px 14px',marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div><div style={{fontSize:13,fontWeight:600,color:'#EDE8DB',marginBottom:2}}>{c.nom}</div><div style={{fontSize:11,color:'rgba(237,232,219,0.4)'}}>{c.courriel||'—'}</div></div>
                    <button onClick={()=>deleteClient(c.id)} style={{background:'rgba(199,91,78,0.08)',border:'none',borderRadius:6,padding:'6px 8px',cursor:'pointer',color:'#C75B4E',display:'flex'}}><Trash2 size={13}/></button>
                  </div>
                ))}</div>
              :<table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
                  {['Nom','Courriel','Téléphone','SIRET',''].map((h,i)=>(<th key={i} style={{padding:'11px 14px',textAlign:'left',fontSize:10,fontWeight:700,color:'rgba(237,232,219,0.4)',textTransform:'uppercase',letterSpacing:'0.06em'}}>{h}</th>))}
                </tr></thead>
                <tbody>{clients.map((c,i)=>(<tr key={c.id||i} style={{borderBottom:'1px solid rgba(255,255,255,0.04)'}} onMouseEnter={ev=>ev.currentTarget.style.background='rgba(255,255,255,0.03)'} onMouseLeave={ev=>ev.currentTarget.style.background='transparent'}>
                  <td style={{padding:'11px 14px',fontSize:13,fontWeight:600,color:'#EDE8DB'}}>{c.nom}</td>
                  <td style={{padding:'11px 14px',fontSize:12,color:'rgba(237,232,219,0.5)'}}>{c.courriel||'—'}</td>
                  <td style={{padding:'11px 14px',fontSize:12,color:'rgba(237,232,219,0.5)'}}>{c['téléphone']||'—'}</td>
                  <td style={{padding:'11px 14px',fontSize:12,color:'rgba(237,232,219,0.5)'}}>{c.siret||'—'}</td>
                  <td style={{padding:'11px 14px'}}><button onClick={()=>deleteClient(c.id)} style={{background:'transparent',border:'none',cursor:'pointer',padding:4,color:'rgba(237,232,219,0.2)'}} onMouseEnter={ev=>ev.currentTarget.style.color='#C75B4E'} onMouseLeave={ev=>ev.currentTarget.style.color='#D0D4DC'}><Trash2 size={13}/></button></td>
                </tr>))}</tbody>
              </table>
          }
        </div>
