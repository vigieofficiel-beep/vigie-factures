...
function TypeBadge({ type }) {
  ...
  const Icon = t.icon;
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:5, background:`${t.color}15`, color:t.color, fontSize:11, fontWeight:600, padding:'3px 9px', borderRadius:20 }}>
...
function AddExpenseForm({ onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error: insErr } = await supabasePro.from('expenses').insert({ user_id:user.id, date:form.date, amount_ttc:form.amount_ttc?parseFloat(form.amount_ttc):null, type:form.type, etablissement:form.etablissement, km, indemnite_km, notes:form.notes, file_url, storage_path });
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background:'#fff', border:'1px solid #E8EAF0', borderRadius:14, padding:24, marginBottom:24, boxShadow:'0 2px 12px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
        <div><label style={lS}>Date *</label><input type="date" value={form.date} onChange={set('date')} required style={{ ...iS, colorScheme:'light' }}/></div>
        <div><label style={lS}>Montant TTC (€) *</label><input type="number" step="0.01" min="0" value={form.amount_ttc} onChange={set('amount_ttc')} placeholder="0,00" required style={iS}/></div>
      </div>
      <div style={{ marginBottom:14 }}>
        <label style={lS}>Catégorie *</label>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          {TYPES.map(t => { const Icon = t.icon; const sel = form.type===t.id; return (
            <button key={t.id} type="button" onClick={() => setForm(f => ({ ...f, type:t.id }))} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 13px', borderRadius:20, border:`1px solid ${sel?t.color:'#E8EAF0'}`, background:sel?`${t.color}15`:'#F8F9FB', color:sel?t.color:'#5A6070', fontSize:12, fontWeight:sel?700:500, cursor:'pointer' }}>
      ...
      {form.type === 'transport' && (
        <div style={{ marginBottom:14, background:'rgba(91,163,199,0.06)', border:'1px solid rgba(91,163,199,0.2)', borderRadius:10, padding:'12px 14px' }}>
          <label style={{ ...lS, color:'#5BA3C7' }}>Kilométrage (facultatif)</label>
...
export default function DepensesPage() {
  ...
  const fetchExpenses = async () => {
    ...
    const { data } = await supabasePro.from('expenses').select('*').eq('user_id', session.user.id).order('date', { ascending:false });
    setExpenses(data || []);
    setLoading(false);
  };
  ...
  return (
    <div style={{ fontFamily:"'Nunito Sans', sans-serif", padding:'32px 28px', maxWidth:900, margin:'0 auto' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24, flexWrap:'wrap', gap:12 }}>
        <div>
          <h1 style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:26, fontWeight:600, color:'#1A1C20', margin:0 }}>Dépenses & Notes de frais</h1>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:18, flexWrap:'wrap' }}>
        {[{ id:'tous', label:'Toutes' }, ...TYPES].map(t => (
          <button key={t.id} onClick={() => setFilterType(t.id)} style={{ padding:'6px 14px', borderRadius:20, border:`1px solid ${filterType===t.id?(t.color||ACCENT):'#E8EAF0'}`, background:filterType===t.id?`${t.color||ACCENT}15`:'#fff', color:filterType===t.id?(t.color||ACCENT):'#5A6070', fontSize:12, fontWeight:filterType===t.id?700:500, cursor:'pointer' }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ background:'#fff', border:'1px solid #E8EAF0', borderRadius:14, overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
        ...
        : filtered.length === 0 ? (
          <div style={{ padding:48, textAlign:'center' }}>
            ...
            <p style={{ color:'#9AA0AE', fontSize:13, margin:0 }}>{expenses.length===0?'Aucune dépense — cliquez sur "Nouvelle dépense"':'Aucune dépense pour ce filtre'}</p>
          </div>
        ) : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><tr style={{ borderBottom:'1px solid #F0F2F5' }}>{['Date','Catégorie','Fournisseur','Montant TTC','Indemnité km','Justificatif',''].map(h=>(<th key={h} style={{ padding:'11px 14px', textAlign:'left', fontSize:10, fontWeight:700, color:'#9AA0AE', textTransform:'uppercase', letterSpacing:'0.06em' }}>{h}</th>))}</tr></thead>
            <tbody>
              {filtered.map((e,i) => (
                <tr key={e.id||i} style={{ borderBottom:'1px solid #F8F9FB' }} onMouseEnter={ev=>ev.currentTarget.style.background='#FAFBFC'} onMouseLeave={ev=>ev.currentTarget.style.background='transparent'}>
                  ...
                  <td style={{ padding:'11px 14px' }}>{e.file_url?<a href={e.file_url} target="_blank" rel="noopener noreferrer" style={{ fontSize:11, color:'#5BA3C7', textDecoration:'none', fontWeight:600 }}>Voir ↗</a>:<span style={{ fontSize:11, color:'#D0D4DC' }}>—</span>}</td>
                  <td style={{ padding:'11px 14px' }}>
                    <button onClick={() => deleteExpense(e.id, e.storage_path)} style={{ background:'transparent', border:'none', cursor:'pointer', padding:4, color:'#D0D4DC' }} onMouseEnter={ev=>ev.currentTarget.style.color='#C75B4E'} onMouseLeave={ev=>ev.currentTarget.style.color='#D0D4DC'}><Trash2 size={14}/></button>
                  </td>
                </tr>
              ))}
            </tbody>
