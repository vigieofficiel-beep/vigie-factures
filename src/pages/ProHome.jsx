    ...
    <div style={{ position: 'relative' }}>
      ...
      <button
        ...
        style={{
          ...
          background: open ? 'rgba(91,163,199,0.1)' : '#fff',
          border: `1px solid ${open ? 'rgba(91,163,199,0.3)' : '#E8EAF0'}`,
          cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 1px 4px rgba(0,0,0,0.06)', transition: 'all 150ms ease',
        }}
        onMouseEnter={e => { if (!open) e.currentTarget.style.borderColor = '#5BA3C7'; }}
        onMouseLeave={e => { if (!open) e.currentTarget.style.borderColor = '#E8EAF0'; }}
      >
        <Bell size={16} color={open ? '#5BA3C7' : '#5A6070'} strokeWidth={2}/>
        {unread > 0 && (
          <div style={{ position: 'absolute', top: -4, right: -4, width: 17, height: 17, borderRadius: '50%', background: '#C75B4E', border: '2px solid #F8FAFC', fontSize: 8, fontWeight: 800, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {unread > 9 ? '9+' : unread}
          </div>
        )}
      </button>
      ...
      {/* Panel notifications */}
      {open && (
        <div ref={panelRef} style={{ position: 'fixed', top: coords.top, left: coords.left, zIndex: 9999, width: 320, maxHeight: 460, background: 'rgba(15,23,42,0.98)', backdropFilter: 'blur(20px)', border: '1px solid rgba(91,163,199,0.2)', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.4)', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'notifSlide 0.18s ease' }}>
  ...
  return (
    <div style={{ background: critiques > 0 ? 'rgba(199,91,78,0.06)' : 'rgba(212,168,83,0.06)', border: `1px solid ${critiques > 0 ? 'rgba(199,91,78,0.25)' : 'rgba(212,168,83,0.25)'}`, borderRadius: 14, marginBottom: 24, overflow: 'hidden' }}>
      <div onClick={() => setCollapsed(c => !c)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', cursor: 'pointer', background: critiques > 0 ? 'rgba(199,91,78,0.04)' : 'rgba(212,168,83,0.04)' }}>
        ...
        <button onClick={e => { e.stopPropagation(); dismissAll(); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94A3B8', fontSize: 11, display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 6 }}>
          ...
          {visibles.map(a => {
            ...
            return (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: s.bg, border: `1px solid ${s.border}`, borderRadius: 9, padding: '9px 12px', marginBottom: 6 }}>
                ...
                <a href={a.lien} style={{ color: '#94A3B8', flexShrink: 0 }}><ChevronRight size={13} /></a>
                <button onClick={() => dismissOne(a.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#CBD5E1', padding: 2, display: 'flex', flexShrink: 0 }} title="Ignorer"><X size={11} /></button>
              </div>
            );
          })}
        </div>
...
export default function ProHome() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: '#94A3B8', fontSize: 14 }}>Chargement…</div>
  );
    ...
    <div style={{ fontFamily: "'Nunito Sans', sans-serif", padding: '32px 28px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#9AA0AE', marginTop: 4, marginBottom: 0 }}>
      ...
      {Object.keys(depensesByType).length > 0 && (
        <div style={{ background: '#fff', border: '1px solid #E8EAF0', borderRadius: 14, padding: 22, marginBottom: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          <h2 style={{ fontSize: 13, fontWeight: 700, color: '#1A1C20', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 16px' }}>Dépenses par catégorie</h2>
