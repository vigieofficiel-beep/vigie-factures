...
function StatutBadge({ rapproche }) {
  return (
    <span style={{
      ...
      background: rapproche ? 'rgba(91,199,138,0.1)' : 'rgba(212,168,83,0.1)',
      color: rapproche ? '#5BC78A' : '#D4A853',
...
export default function BanquePage() {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (parsed.length === 0) {
      alert('Aucune transaction trouvée. Vérifiez le format du fichier CSV (colonnes : Date;Libellé;Montant).');
      return;
    }
  ...
  const confirmImport = async () => {
    ...
    const withRapprochement = preview.map(tx => {
      ...
      return {
        ...
        rapproche : !!matched,
        statut    : matched ? 'rapproche' : 'non_rapproche',
      };
    });
  ...
  const toggleRapprochement = async (tx) => {
    ...
    await supabasePro.from('bank_transactions').update({
      rapproche: newVal,
      statut   : newVal ? 'rapproche' : 'non_rapproche',
    }).eq('id', tx.id);
    fetchAll();
  };
  ...
  const exportCSV = () => {
    const rows = filtered.map(t => [t.date, t.libelle, t.montant, t.type, t.rapproche ? 'Rapproché' : 'Non rapproché']);
    const headers = ['Date', 'Libellé', 'Montant', 'Type', 'Statut'];
    const csv = [headers, ...rows].map(r => r.map(v => `"${v ?? ''}"`).join(';')).join('\n');
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8' }));
    a.download = `banque-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
      ...
      {preview && (
        <div style={{ background: '#fff', border: '1px solid rgba(91,163,199,0.3)', borderRadius: 14, padding: 20, marginBottom: 24, boxShadow: '0 2px 12px rgba(91,163,199,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: '#1A1C20', margin: 0 }}>
                Aperçu — {preview.length} transactions détectées
              </h3>
              <p style={{ fontSize: 12, color: '#9AA0AE', marginTop: 4 }}>
                Le rapprochement automatique sera tenté avec vos factures existantes.
              </p>
            </div>
            <button onClick={() => setPreview(null)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#9AA0AE' }}>
              <X size={16} />
            </button>
          </div>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid #F0F2F5' }}>
                  {['Date', 'Libellé', 'Montant'].map(h => (
                    <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: '#9AA0AE', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {preview.slice(0, 10).map((t, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #F8F9FB' }}>
                    <td style={{ padding: '7px 12px', color: '#5A6070' }}>{formatDate(t.date)}</td>
                    <td style={{ padding: '7px 12px', color: '#1A1C20' }}>{t.libelle}</td>
                    <td style={{ padding: '7px 12px', fontWeight: 700, color: t.montant >= 0 ? '#5BC78A' : '#C75B4E' }}>
                      {formatEuro(t.montant)}
                    </td>
                  </tr>
                ))}
                {preview.length > 10 && (
                  <tr><td colSpan={3} style={{ padding: '8px 12px', color: '#9AA0AE', fontSize: 11, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={confirmImport} disabled={importing} style={{ flex: 1, padding: '11px', borderRadius: 9, border: 'none', background: importing ? `${ACCENT}50` : ACCENT, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              {importing ? 'Import en cours...' : `✓ Confirmer l'import (${preview.length} transactions)`}
            </button>
            <button onClick={() => setPreview(null)} style={{ padding: '11px 18px', borderRadius: 9, border: '1px solid #E8EAF0', background: '#fff', color: '#5A6070', fontSize: 13, cursor: 'pointer' }}>
              Annuler
            </button>
          </div>
        </div>
      ...
      {transactions.length === 0 && !preview && (
        <div style={{ background: 'rgba(91,163,199,0.04)', border: '1px solid rgba(91,163,199,0.15)', borderRadius: 12, padding: '16px 20px', marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 12, color: '#5A6070', margin: 0 }}>
            Le fichier doit avoir 3 colonnes séparées par <code>;</code> ou <code>,</code> :<br />
            <strong>Date ; Libellé ; Montant</strong><br />
            Exemple : <code>01/03/2026;EDF FACTURE;-124.50</code>
          </p>
        </div>
      )}
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 6 }}>
            {[{ id: 'tous', label: 'Tous' }, { id: 'credit', label: 'Crédits' }, { id: 'debit', label: 'Débits' }].map(f => (
              <button key={f.id} onClick={() => setFilterType(f.id)} style={{
                padding: '5px 14px', borderRadius: 20, border: `1px solid ${filterType === f.id ? ACCENT : '#E8EAF0'}`,
                background: filterType === f.id ? `${ACCENT}15` : '#fff',
                color: filterType === f.id ? ACCENT : '#5A6070',
                fontSize: 12, fontWeight: filterType === f.id ? 700 : 500, cursor: 'pointer',
              }}>{f.label}</button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[{ id: 'tous', label: 'Tous' }, { id: 'rapproche', label: 'Rapprochés' }, { id: 'non_rapproche', label: 'À rapprocher' }].map(f => (
              <button key={f.id} onClick={() => setFilterRaproche(f.id)} style={{
                padding: '5px 14px', borderRadius: 20, border: `1px solid ${filterRaproche === f.id ? '#D4A853' : '#E8EAF0'}`,
                background: filterRaproche === f.id ? 'rgba(212,168,83,0.1)' : '#fff',
                color: filterRaproche === f.id ? '#D4A853' : '#5A6070',
                fontSize: 12, fontWeight: filterRaproche === f.id ? 700 : 500, cursor: 'pointer',
              }}>{f.label}</button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: 48, textAlign: 'center' }}>
            ...
            <p style={{ color: '#9AA0AE', fontSize: 13, margin: 0 }}>
              {transactions.length === 0 ? 'Aucune transaction — importez votre relevé bancaire CSV' : 'Aucune transaction pour ce filtre'}
            </p>
          </div>
        ) : (
            ...
            <tbody>
              {filtered.map((t, i) => (
                ...
                >
                  ...
                  <td style={{ padding: '11px 14px', fontSize: 13, color: '#1A1C20', maxWidth: 300 }}>
                    <span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {t.libelle}
                    </span>
                  </td>
                  <td style={{ padding: '11px 14px', fontSize: 13, fontWeight: 700, color: t.montant >= 0 ? '#5BC78A' : '#C75B4E' }}>
                  ...
                  <td style={{ padding: '11px 14px' }}>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button
                        onClick={() => toggleRapprochement(t)}
                        title={t.rapproche ? 'Marquer comme non rapproché' : 'Marquer comme rapproché'}
                        style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 4, color: t.rapproche ? '#5BC78A' : '#D4A853' }}
                      >
                        <RefreshCw size={13} />
                      </button>
