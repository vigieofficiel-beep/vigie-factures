...
import {
  Plus, Trash2, Edit2, FileCheck, AlertTriangle, CheckCircle,
  Clock, Download, Upload, Bell
} from 'lucide-react';
...
function ContratForm({ onSave, onCancel, editData = null }) {
  const [form, setForm] = useState(editData || {
    nom: '', type: 'assurance', fournisseur: '',
    date_debut: '', date_fin: '',
    montant_periodique: '', periodicite: 'mensuel',
    reconduction_tacite: false, delai_preavis_jours: 30,
    statut: 'actif', notes: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...form,
        user_id            : user.id,
        montant_periodique : form.montant_periodique ? parseFloat(form.montant_periodique) : null,
        delai_preavis_jours: form.delai_preavis_jours ? parseInt(form.delai_preavis_jours) : null,
        file_url, storage_path,
      };

      if (editData?.id) {
        const { error: err } = await supabasePro.from('contrats').update(payload).eq('id', editData.id);
        if (err) throw err;
      } else {
        const { error: err } = await supabasePro.from('contrats').insert(payload);
        if (err) throw err;
      }
      onSave();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background: '#fff', border: '1px solid #E8EAF0', borderRadius: 14, padding: 24, marginBottom: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
      <h3 style={{ fontSize: 15, fontWeight: 700, color: '#1A1C20', marginBottom: 20 }}>
        {editData ? 'Modifier le contrat' : 'Ajouter un contrat'}
      </h3>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        <div>
          <label style={labelStyle}>Nom du contrat *</label>
          <input value={form.nom} onChange={set('nom')} required style={inputStyle} placeholder="Ex: Assurance RC Pro" />
        </div>
        <div>
        ...
        <div>
          <label style={labelStyle}>Fournisseur / Prestataire</label>
          <input value={form.fournisseur} onChange={set('fournisseur')} style={inputStyle} placeholder="Ex: AXA, Orange..." />
        </div>
        <div>
          <label style={labelStyle}>Périodicité</label>
          <select value={form.periodicite} onChange={set('periodicite')} style={{ ...inputStyle, cursor: 'pointer' }}>
            {PERIODICITES.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Date de début</label>
          <input type="date" value={form.date_debut} onChange={set('date_debut')} style={{ ...inputStyle, colorScheme: 'light' }} />
        </div>
        <div>
          <label style={labelStyle}>Date de fin / Échéance</label>
          <input type="date" value={form.date_fin} onChange={set('date_fin')} style={{ ...inputStyle, colorScheme: 'light' }} />
        </div>
        <div>
          <label style={labelStyle}>Montant périodique (€)</label>
          <input type="number" step="0.01" min="0" value={form.montant_periodique} onChange={set('montant_periodique')} style={inputStyle} placeholder="0.00" />
        </div>
        <div>
          <label style={labelStyle}>Délai de préavis (jours)</label>
          <input type="number" min="0" value={form.delai_preavis_jours} onChange={set('delai_preavis_jours')} style={inputStyle} placeholder="30" />
        </div>
      </div>
      ...
      <div style={{ background: form.reconduction_tacite ? 'rgba(199,91,78,0.04)' : '#F8F9FB', border: `1px solid ${form.reconduction_tacite ? 'rgba(199,91,78,0.2)' : '#E8EAF0'}`, borderRadius: 10, padding: '12px 16px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
        <input type="checkbox" id="reconduction" checked={form.reconduction_tacite} onChange={setCheck('reconduction_tacite')}
          style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#C75B4E' }} />
        <label htmlFor="reconduction" style={{ fontSize: 13, color: form.reconduction_tacite ? '#C75B4E' : '#5A6070', fontWeight: form.reconduction_tacite ? 600 : 400, cursor: 'pointer' }}>
          Reconduction tacite
          {form.reconduction_tacite && <span style={{ fontSize: 11, marginLeft: 8, opacity: 0.7 }}>— Une alerte sera générée {form.delai_preavis_jours}j avant l'échéance</span>}
        </label>
      </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <textarea value={form.notes} onChange={set('notes')} rows={2}
          ...
          placeholder="Conditions particulières, contacts, références..." />
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div onClick={() => fileRef.current?.click()} style={{
          ...
          background: file ? `${ACCENT}08` : '#F8F9FB',
        }}>
          ...
          {file ? (
            ...
            <span style={{ fontSize: 12, color: '#5BC78A', fontWeight: 600 }}>✓ Document existant — cliquez pour remplacer</span>
          ) : (
            <span style={{ fontSize: 12, color: '#9AA0AE' }}>Cliquez pour ajouter le contrat PDF</span>
          )}
        </div>
      </div>
...
export default function ContratsPage() {
  ...
  const exportCSV = () => {
    const rows = contrats.map(c => [c.nom, c.type, c.fournisseur, c.date_debut, c.date_fin, c.montant_periodique, c.periodicite, c.reconduction_tacite ? 'Oui' : 'Non', c.delai_preavis_jours]);
    const headers = ['Nom', 'Type', 'Fournisseur', 'Début', 'Fin', 'Montant', 'Périodicité', 'Reconduction tacite', 'Préavis (jours)'];
  ...
  const totalAnnuel = contrats.reduce((s, c) => {
    ...
    if (c.periodicite === 'annuel')       return s + m;
    return s + m;
  }, 0);
    ...
    <div style={{ fontFamily: "'Nunito Sans', sans-serif", padding: '32px 28px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      {(alertes.length > 0 || expires.length > 0) && (
        <div style={{ background: 'rgba(199,91,78,0.04)', border: '1px solid rgba(199,91,78,0.2)', borderRadius: 12, padding: '14px 18px', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: '#C75B4E' }}>Alertes échéances</span>
          </div>
          {expires.map(c => (
            <p key={c.id} style={{ fontSize: 12, color: '#C75B4E', margin: '3px 0' }}>
              ⚠️ <strong>{c.nom}</strong> ({c.fournisseur}) — expiré depuis {Math.abs(daysUntil(c.date_fin))} jours
            </p>
          ))}
          {alertes.map(c => (
            <p key={c.id} style={{ fontSize: 12, color: '#D4A853', margin: '3px 0' }}>
              ⏰ <strong>{c.nom}</strong> ({c.fournisseur}) — expire dans {daysUntil(c.date_fin)} jours
              {c.reconduction_tacite && <span style={{ marginLeft: 6, fontSize: 11, opacity: 0.8 }}>— reconduction tacite, préavis {c.delai_preavis_jours}j</span>}
            </p>
          ))}
        </div>
      )}
      ...
      {(showForm || editContrat) && (
        <ContratForm
          ...
          onSave={() => { setShowForm(false); setEditContrat(null); fetchContrats(); }}
          onCancel={() => { setShowForm(false); setEditContrat(null); }}
        />
      )}
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        {[{ id: 'tous', label: 'Tous' }, ...TYPES].map(t => (
          <button key={t.id} onClick={() => setFilterType(t.id)} style={{
            ...
            border: `1px solid ${filterType === t.id ? ACCENT : '#E8EAF0'}`,
            background: filterType === t.id ? `${ACCENT}15` : '#fff',
            color: filterType === t.id ? ACCENT : '#5A6070',
      ...
      <div style={{ background: '#fff', border: '1px solid #E8EAF0', borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        {loading ? (
          ...
          <div style={{ padding: 48, textAlign: 'center' }}>
            <FileCheck size={32} color="#E8EAF0" style={{ marginBottom: 12 }} />
            <p style={{ color: '#9AA0AE', fontSize: 13, margin: 0 }}>
              {contrats.length === 0 ? 'Aucun contrat — cliquez sur "Ajouter un contrat"' : 'Aucun contrat pour ce filtre'}
            </p>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #F0F2F5' }}>
                {['Contrat', 'Fournisseur', 'Type', 'Montant', 'Échéance', 'Statut', ''].map(h => (
              ...
              {filtered.map((c, i) => (
                ...
                >
                  ...
                  <td style={{ padding: '11px 14px', fontSize: 12, color: '#5A6070' }}>
                    {TYPES.find(t => t.id === c.type)?.label || c.type}
                  </td>
                  <td style={{ padding: '11px 14px', fontSize: 13, fontWeight: 600, color: ACCENT }}>
                    {formatEuro(c.montant_periodique)}
                    {c.periodicite && <span style={{ fontSize: 10, color: '#9AA0AE', fontWeight: 400, marginLeft: 4 }}>/{c.periodicite}</span>}
                  </td>
                  ...
                  <td style={{ padding: '11px 14px' }}>
                    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                      {c.file_url && (
                        <a href={c.file_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: '#5BA3C7', textDecoration: 'none', fontWeight: 600, padding: 4 }}>PDF ↗</a>
                      )}
                      <button onClick={() => { setEditContrat(c); setShowForm(false); }}
                      ...
                      <button onClick={() => deleteContrat(c.id, c.storage_path)}
