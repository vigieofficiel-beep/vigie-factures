...
function StatutBadge({ contrat }) {
  ...
  if (days <= 30) return <span style={{ background:'rgba(199,91,78,0.1)',  color:'#C75B4E', fontSize:11, fontWeight:700, padding:'3px 10px', borderRadius:20 }}>⚠️ {days}j</span>;
  if (days <= 60) return <span style={{ background:'rgba(212,168,83,0.1)', color:'#D4A853', fontSize:11, fontWeight:700, padding:'3px 10px', borderRadius:20 }}>⏰ {days}j</span>;
...
function ContratForm({ onSave, onCancel, editData=null, prefill=null, workspaceId=null }) {
  const [form, setForm] = useState(editData || {
    nom: prefill?.nom_contrat ?? '', type: 'assurance', fournisseur: prefill?.fournisseur ?? '',
    date_debut: prefill?.date ? parseOCRDate(prefill.date) : '', date_fin: prefill?.date_fin ? parseOCRDate(prefill.date_fin) : '',
    montant_periodique: prefill?.montant_ttc ?? '', periodicite: 'mensuel',
    reconduction_tacite: false, delai_preavis_jours: 30, statut: 'actif', notes: prefill?.description ?? '',
  });
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background:'rgba(255,255,255,0.04)', border:`1px solid ${prefill?`${ACCENT}40`:'rgba(255,255,255,0.08)'}`, borderRadius:14, padding:24, marginBottom:24 }}>
      ...
      <h3 style={{ fontSize:15, fontWeight:700, color:'#EDE8DB', marginBottom:20 }}>{editData?'Modifier le contrat':'Ajouter un contrat'}</h3>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
        ...
        <div><label style={lS}>Fournisseur</label><input value={form.fournisseur} onChange={set('fournisseur')} style={iS} placeholder="Ex: AXA, Orange..."/></div>
        ...
        <div><label style={lS}>Date de fin</label><input type="date" value={form.date_fin} onChange={set('date_fin')} style={{...iS,colorScheme:'dark'}}/></div>
        ...
        <div><label style={lS}>Délai de préavis (jours)</label><input type="number" min="0" value={form.delai_preavis_jours} onChange={set('delai_preavis_jours')} style={iS} placeholder="30"/></div>
      </div>
      <div style={{ background:form.reconduction_tacite?'rgba(199,91,78,0.06)':'rgba(255,255,255,0.03)', border:`1px solid ${form.reconduction_tacite?'rgba(199,91,78,0.2)':'rgba(255,255,255,0.08)'}`, borderRadius:10, padding:'12px 16px', marginBottom:14, display:'flex', alignItems:'center', gap:12 }}>
        ...
        <label htmlFor="reconduction" style={{ fontSize:13, color:form.reconduction_tacite?'#C75B4E':'rgba(237,232,219,0.5)', fontWeight:form.reconduction_tacite?600:400, cursor:'pointer' }}>
          Reconduction tacite {form.reconduction_tacite&&<span style={{ fontSize:11, opacity:0.7 }}>— Alerte {form.delai_preavis_jours}j avant</span>}
        </label>
      </div>
...
export default function ContratsPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) { const data=JSON.parse(raw); if(data.source==='prohome_ocr'&&data.type_document==='contrat'){setOcrPrefill(data);setShowForm(true);sessionStorage.removeItem('ocr_prefill');} }
    } catch {}
  }, [activeWorkspace]);
  ...
  const fetchContrats = async () => {
    ...
    const { data } = await supabasePro.from('contrats').select('*').eq('user_id',user.id).eq('workspace_id',activeWorkspace?.id).order('date_fin',{ascending:true});
    setContrats(data||[]); setLoading(false);
  };
  ...
  const totalAnnuel = contrats.reduce((s,c)=>{const m=c.montant_periodique||0;if(c.periodicite==='mensuel')return s+m*12;if(c.periodicite==='trimestriel')return s+m*4;return s+m;},0);
  ...
  return (
    <div style={{ fontFamily:"'Nunito Sans', sans-serif", padding:isMobile?'16px 12px':'32px 28px', maxWidth:1000, margin:'0 auto' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <h1 style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:isMobile?22:26, fontWeight:600, color:'#EDE8DB', margin:0 }}>Contrats & Assurances</h1>
        <div style={{ display:'flex', gap:8 }}>
          {!isMobile&&<DateFilter onChange={setDateRange} color={ACCENT}/>}
          {!isMobile&&<ExportButton data={contrats.map(c=>({...c,reconduction_label:c.reconduction_tacite?'Oui':'Non'}))} filename={`contrats-${new Date().getFullYear()}`} color={ACCENT} columns={[{key:'nom',label:'Nom'},{key:'type',label:'Type'},{key:'fournisseur',label:'Fournisseur'},{key:'date_fin',label:'Fin'},{key:'montant_periodique',label:'Montant (€)'},{key:'periodicite',label:'Périodicité'},{key:'reconduction_label',label:'Reconduction'}]}/>}
          <button onClick={()=>{setShowForm(true);setEditContrat(null);setOcrPrefill(null);}} style={{ display:'flex', alignItems:'center', gap:6, padding:'9px 14px', borderRadius:9, border:'none', background:ACCENT, color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer' }}>
            <Plus size={13}/> {isMobile?'Contrat':'Ajouter un contrat'}
          </button>
        </div>
      </div>
      ...
      {(alertes.length>0||expires.length>0)&&(
        <div style={{ background:'rgba(199,91,78,0.06)', border:'1px solid rgba(199,91,78,0.2)', borderRadius:12, padding:'14px 18px', marginBottom:20 }}>
          ...
          {expires.map(c=><p key={c.id} style={{fontSize:12,color:'#C75B4E',margin:'3px 0'}}>⚠️ <strong>{c.nom}</strong> — expiré depuis {Math.abs(daysUntil(c.date_fin))}j</p>)}
          {alertes.map(c=><p key={c.id} style={{fontSize:12,color:'#5BC78A',margin:'3px 0'}}>⏰ <strong>{c.nom}</strong> — expire dans {daysUntil(c.date_fin)}j{c.reconduction_tacite&&<span style={{marginLeft:6,fontSize:11,opacity:0.8}}>— reconduction tacite</span>}</p>)}
        </div>
      )}

      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(130px, 1fr))', gap:10, marginBottom:20 }}>
        {[
          {label:'Actifs',      value:contrats.length,        color:ACCENT,    icon:FileCheck},
          ...
          {label:'Coût annuel', value:formatEuro(totalAnnuel), color:'#5BC78A', icon:CheckCircle},
        ].map((s,idx)=>{const Icon=s.icon;return(
          <div key={idx} style={{ background:'rgba(255,255,255,0.04)', border:`1px solid ${s.alert?'rgba(199,91,78,0.3)':'rgba(255,255,255,0.08)'}`, borderRadius:12, padding:'12px 14px' }}>
      ...
      {(showForm||editContrat)&&<ContratForm editData={editContrat} prefill={ocrPrefill} workspaceId={activeWorkspace?.id} onSave={()=>{setShowForm(false);setEditContrat(null);setOcrPrefill(null);fetchContrats();}} onCancel={()=>{setShowForm(false);setEditContrat(null);setOcrPrefill(null);}}/>}

      <div style={{ display:'flex', gap:6, marginBottom:14, flexWrap:'wrap' }}>
        {[{id:'tous',label:'Tous'},...TYPES].map(t=>(
          <button key={t.id} onClick={()=>setFilterType(t.id)} style={{ padding:'5px 12px', borderRadius:20, border:`1px solid ${filterType===t.id?ACCENT:'rgba(255,255,255,0.1)'}`, background:filterType===t.id?`${ACCENT}20`:'transparent', color:filterType===t.id?ACCENT:'rgba(237,232,219,0.4)', fontSize:11, fontWeight:filterType===t.id?700:500, cursor:'pointer', whiteSpace:'nowrap' }}>{t.label}</button>
        ))}
      </div>
      ...
      {!isMobile&&(
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, overflow:'hidden' }}>
          ...
          :filtered.length===0?<div style={{padding:48,textAlign:'center'}}><FileCheck size={32} color="rgba(255,255,255,0.1)" style={{marginBottom:12}}/><p style={{color:'rgba(237,232,219,0.3)',fontSize:13,margin:0}}>{contrats.length===0?'Aucun contrat — cliquez sur "Ajouter un contrat"':'Aucun contrat pour ce filtre'}</p></div>
          :<table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
              {['Contrat','Fournisseur','Type','Montant','Échéance','Statut',''].map((h,i)=>(<th key={i} style={{padding:'11px 14px',textAlign:'left',fontSize:10,fontWeight:700,color:'rgba(237,232,219,0.3)',textTransform:'uppercase',letterSpacing:'0.06em'}}>{h}</th>))}
            </tr></thead>
            <tbody>{filtered.map((c,i)=>(
              <tr key={c.id||i} style={{borderBottom:'1px solid rgba(255,255,255,0.04)'}} onMouseEnter={ev=>ev.currentTarget.style.background='rgba(255,255,255,0.03)'} onMouseLeave={ev=>ev.currentTarget.style.background='transparent'}>
                <td style={{padding:'11px 14px'}}><p style={{fontSize:13,fontWeight:600,color:'#EDE8DB',margin:0}}>{c.nom}</p>{c.reconduction_tacite&&<p style={{fontSize:10,color:'#C75B4E',margin:'2px 0 0'}}>Reconduction tacite — préavis {c.delai_preavis_jours}j</p>}</td>
                ...
                <td style={{padding:'11px 14px',fontSize:13,fontWeight:600,color:ACCENT}}>{formatEuro(c.montant_periodique)}{c.periodicite&&<span style={{fontSize:10,color:'rgba(237,232,219,0.3)',marginLeft:4}}>/{c.periodicite}</span>}</td>
      ...
      {isMobile&&(
        <div>
          ...
          :filtered.length===0?<div style={{padding:40,textAlign:'center'}}><p style={{color:'rgba(237,232,219,0.3)',fontSize:13,margin:0}}>{contrats.length===0?'Aucun contrat — appuyez sur "Contrat"':'Aucun contrat pour ce filtre'}</p></div>
          :filtered.map((c,i)=>(
            <div key={c.id||i} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, padding:'14px 16px', marginBottom:10 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                <div>
                  <div style={{ fontSize:13, fontWeight:700, color:'#EDE8DB', marginBottom:2 }}>{c.nom}</div>
                  <div style={{ fontSize:11, color:'rgba(237,232,219,0.4)' }}>{c.fournisseur||'—'} · {TYPES.find(t=>t.id===c.type)?.label||c.type}</div>
                </div>
                <StatutBadge contrat={c}/>
              </div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div>
                  <div style={{ fontSize:14, fontWeight:700, color:ACCENT }}>{formatEuro(c.montant_periodique)}{c.periodicite&&<span style={{ fontSize:10, color:'rgba(237,232,219,0.3)', marginLeft:4 }}>/{c.periodicite}</span>}</div>
                  <div style={{ fontSize:11, color:'rgba(237,232,219,0.4)', marginTop:2 }}>Fin : {formatDate(c.date_fin)}</div>
                </div>
                <div style={{ display:'flex', gap:6 }}>
                  {c.file_url&&<button onClick={()=>window.open(c.file_url,'_blank')} style={{background:'rgba(91,163,199,0.1)',border:'none',borderRadius:6,padding:'6px 8px',cursor:'pointer',color:'#5BA3C7',display:'flex',alignItems:'center',gap:4,fontSize:11,fontWeight:600}}><ExternalLink size={11}/> PDF</button>}
                  <button onClick={()=>{setEditContrat(c);setShowForm(false);}} style={{background:'rgba(255,255,255,0.06)',border:'none',borderRadius:6,padding:'6px 8px',cursor:'pointer',color:'rgba(237,232,219,0.5)',display:'flex'}}><Edit2 size={13}/></button>
                  <button onClick={()=>deleteContrat(c.id,c.storage_path)} style={{background:'rgba(199,91,78,0.08)',border:'none',borderRadius:6,padding:'6px 8px',cursor:'pointer',color:'#C75B4E',display:'flex'}}><Trash2 size={13}/></button>
                </div>
              </div>
              {c.reconduction_tacite&&<div style={{marginTop:8,fontSize:11,color:'#C75B4E'}}>⚠️ Reconduction tacite — préavis {c.delai_preavis_jours}j</div>}
            </div>
          ))}
        </div>
      )}
