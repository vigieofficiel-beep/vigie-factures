...
const ETAPES = [
  {
    ...
    description: 'Ajoutez votre SIRET, adresse et IBAN pour les devis et factures.',
    route: '/pro/profil',
    emoji: '👤',
    important: true,
  },
  {
    id: 'depense',
    titre: 'Ajouter ma première dépense',
    description: 'Enregistrez une dépense pour découvrir le module.',
    route: '/pro/depenses',
    emoji: '🧾',
    important: true,
  },
  {
    ...
    description: 'Générez un devis PDF professionnel en quelques clics.',
    route: '/pro/recettes',
    emoji: '📋',
    important: true,
  },
  {
    ...
    description: 'Connectez votre banque via import CSV pour le rapprochement.',
    route: '/pro/banque',
    emoji: '🏦',
    important: false,
  },
  {
    id: 'contrat',
    titre: 'Ajouter un contrat ou assurance',
    description: 'Suivez vos échéances et recevez des alertes automatiques.',
    route: '/pro/contrats',
    emoji: '📄',
    important: false,
  },
  {
    id: 'formalite',
    titre: 'Vérifier mes obligations légales',
    description: 'Consultez vos formalités obligatoires et leurs échéances.',
    route: '/pro/formalites',
    emoji: '⚖️',
    important: false,
  },
  {
    id: 'vigil',
    titre: 'Poser une question à Vigil',
    description: 'Testez votre assistant IA en bas à droite de l\'écran.',
    route: null,
    emoji: '🤖',
    important: false,
  },
...
export default function OnboardingChecklist() {
  ...
  useEffect(() => {
    try {
      ...
      if (data.ferme) return; // définitivement fermé
  ...
  const sauvegarder = (newChecked, ferme = false) => {
    try {
      localStorage.setItem(CLE_STORAGE, JSON.stringify({ checked: newChecked, ferme }));
    } catch {}
  };
  ...
  const fermerDefinitivement = () => {
    sauvegarder(checked, true);
    setVisible(false);
  };

  const allerVers = (etape) => {
    if (!checked[etape.id]) {
      const next = { ...checked, [etape.id]: true };
      setChecked(next);
      sauvegarder(next);
    }
  ...
  const termине  = done === total;

  // Version minimisée — juste un badge en bas à gauche
  if (minimise) {
    return (
      <div
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '16px 18px', background: 'linear-gradient(135deg, rgba(91,163,199,0.15), rgba(91,163,199,0.05))', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: '#F8FAFC' }}>
              {termине ? '🎉 Félicitations !' : 'Guide de démarrage'}
            </span>
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={() => setMinimise(true)} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', color: '#94A3B8', fontSize: 11 }}>
              Réduire
            </button>
            <button onClick={fermerDefinitivement} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: 6, padding: 4, cursor: 'pointer', color: '#94A3B8', display: 'flex' }}>
              <X size={14}/>
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1, height: 6, background: 'rgba(255,255,255,0.08)', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ width: `${pct}%`, height: '100%', background: termине ? 'linear-gradient(90deg, #5BC78A, #3da86a)' : 'linear-gradient(90deg, #5BA3C7, #3d7fa8)', borderRadius: 3, transition: 'width 0.5s ease' }}/>
          </div>
          <span style={{ fontSize: 12, fontWeight: 700, color: termине ? '#5BC78A' : '#5BA3C7', whiteSpace: 'nowrap' }}>{done}/{total}</span>
        </div>

        {termине && (
          <p style={{ fontSize: 12, color: '#5BC78A', marginTop: 8, marginBottom: 0 }}>
            Vous avez complété toutes les étapes ! Vigie Pro est prêt. 🚀
          </p>
        )}
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: fait ? 'rgba(91,199,138,0.05)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${fait ? 'rgba(91,199,138,0.15)' : 'rgba(255,255,255,0.06)'}`,
            ...
            >
              ...
              <button onClick={() => toggleCheck(etape.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex', flexShrink: 0 }}>
                {fait
                  ? <CheckCircle size={20} color="#5BC78A" strokeWidth={2}/>
                  : <Circle size={20} color="rgba(255,255,255,0.2)" strokeWidth={2}/>
                }
              </button>
              ...
              <div style={{ flex: 1, minWidth: 0 }} onClick={() => allerVers(etape)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: fait ? 500 : 700, color: fait ? 'rgba(255,255,255,0.4)' : '#F8FAFC', textDecoration: fait ? 'line-through' : 'none' }}>
                    {etape.titre}
                  </span>
      ...
      <div style={{ padding: '10px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
        <button onClick={fermerDefinitivement} style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
